...
class CreditTest extends TestCase
{
    ...
    public function test_create_credit(){
        ...
        $header['Accept'] = "application/json";
        $header['Authorization'] = "Bearer ".$GLOBALS['l_premium']['token'];
        $response = $this->post(url("/api/v1/credit/"), $body, $header);
        $GLOBALS['v_premium'] = json_decode($response->content(),true);
        $response->assertStatus(200);
    }
    ...
    public function test_update_credit(){
        ...
        $header['Accept'] = "application/json";
        $header['Authorization'] = "Bearer ".$GLOBALS['l_premium']['token'];
        $response = $this->put(url("/api/v1/credit/".$GLOBALS['v_premium']['id']), $body, $header);
        $response->assertStatus(200);
    }
    ...
    public function test_delete_credit(){
        ...
        $body = [];
        $header['Authorization'] = "Bearer ".$GLOBALS['l_premium']['token'];
        $response = $this->delete(url("/api/v1/credit/".$GLOBALS['v_premium']['id']),$body, $header);
        $response->assertStatus(200);
    }
